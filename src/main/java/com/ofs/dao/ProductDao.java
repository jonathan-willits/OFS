    ...
    private List<Product> getListProductQuery(String query) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                list.add(new Product(id, name, base64Image, price, description, category, account, isDelete, amount, weight));
            }
    ...
    public Product getProduct(int productId) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                product.setWeight(resultSet.getDouble(10));
            }
        } catch (SQLException | ClassNotFoundException | IOException e) {
            System.out.println(e.getMessage());
        }
...
//        }
    ...
    public void addProduct(String productName, InputStream productImage, double productPrice, String productDescription, int productCategory, int sellerId, int productAmount, double productWeight) {
        String query = "INSERT INTO product (product_name, product_image, product_price, product_description, fk_category_id, fk_account_id, product_is_deleted, product_amount, product_weight) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            connection = new Database().getConnection();
            ...
            preparedStatement.setDouble(9, productWeight);
            preparedStatement.executeUpdate();
        } catch (ClassNotFoundException | SQLException e) {
            System.out.println(e.getMessage());
        }
...
//        }
    ...
    public void editProduct(int productId, String productName, InputStream productImage, double productPrice, String productDescription, int productCategory, int productAmount, double productWeight) {
        String query = "UPDATE product SET product_name = ?, product_image = ?, product_price = ?, product_description = ?, fk_category_id = ?, product_amount = ?, product_weight = ? WHERE product_id = ?";
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            connection = new Database().getConnection();
            preparedStatement = connection.prepareStatement(query);
            ...
            preparedStatement.setDouble(8, productWeight);
            preparedStatement.executeUpdate();
        } catch (ClassNotFoundException | SQLException e) {
            System.out.println(e.getMessage());
        }
